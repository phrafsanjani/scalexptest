...
where $\theta > 0, r, m \in \mathbb{R} \setminus \{0\} \text{, and } x \in I$ with $I \subset \mathbb{R}$ continuous and independent of $\theta$ (@singh_sequence-compound_1978).
...
The previous test can be utilized in testing one-sided hypotheses (see Thorem 2 in the paper).
...
since we also want to calculate the value of power of this test at $\theta = 1.25$:
...
where the vector of critical values $c_1$ and $c_2$ are computed by calling `interval_alt_critvals(theta1, theta2, r, n, m, alpha)`.
...
the test function
$$
\phi(\symbfit{X}) = \mathbb{I}(Q_1 < c_1 \mbox{ or } c_2 > Q_1)
$$
is a UMPU level $\alpha$ test for this problem,
where the vector of critical values $c_1$ and $c_2$ are computed by calling `interval_null_critvals(theta1, theta2, r, n, m, alpha)`.
